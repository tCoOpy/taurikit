...
description: Solutions for common TauriKit issues on Windows, macOS, and Linux.
---
...
TauriKit requires Rust ≥ 1.88. Update via rustup:
...
1. Delete the template cache:
   - **macOS/Linux**: `rm -rf ~/.cache/taurikit/templates/`
   - **Windows**: Delete `%APPDATA%\taurikit\templates\`
2. Re-run `taurikit new`
...
1. Make sure _both_ the Vite dev server and Rust backend are running (`bun tauri dev` starts both)
