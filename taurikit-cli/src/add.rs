...
const FEATURES: &[FeatureInfo] = &[
    ...
    FeatureInfo {
        ...
        capabilities: &["clipboard-manager:allow-read", "clipboard-manager:allow-write"],
    },
    FeatureInfo {
...
pub fn run(config: Config) -> Result<()> {
    ...
    let feature = FEATURES
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Unknown feature '{}'. Run `taurikit add list` to see available features.",
                config.feature
            )
        })?;
    ...
    if !project.join("package.json").exists() || !project.join("src-tauri").exists() {
        anyhow::bail!(
            "Not a TauriKit project. Run this command from a TauriKit project directory."
        );
    }
